  ...
  const doSearch = async (mode: SearchMode, value: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (d.deleted) { setDeletedBooking(true); return; }
        if (d.notFound) { setBookingNotFound(true); return; }
        throw new Error(d.error || 'Not found');
      }
      const data = await res.json();
      setBooking(data);
    } catch (err: unknown) {
      if (!deletedBooking && !bookingNotFound) {
            ...
            <>
              ...
              <form onSubmit={handleTrack} className="max-w-md mx-auto mb-10">
                <div className="flex border border-gray-200 rounded-xl overflow-hidden bg-white mb-3">
                    <button type="button" onClick={() => { setSearchMode('bookingId'); setSearchValue(''); setBooking(null); setError(''); setDeletedBooking(false); setBookingNotFound(false); }}
                    ...
                    <button type="button" onClick={() => { setSearchMode('phone'); setSearchValue(''); setBooking(null); setError(''); setDeletedBooking(false); setBookingNotFound(false); }}
                ...
                <div className="flex gap-3">
                  <input
                    ...
                    value={searchValue}
                    onChange={(e) => { setSearchValue(e.target.value); setDeletedBooking(false); setBookingNotFound(false); }}
...
{bookingNotFound && (
  <div className="bg-red-50 border border-red-200 text-center rounded-xl px-5 py-6 mb-6">
    <p className="text-2xl mb-2">❌</p>
    <p className="text-red-700 font-semibold text-[17px] mb-1">Booking Not Found</p>
    <p className="text-red-600 text-[14px] mb-4">The Booking ID you entered does not exist.<br />Please check the Booking ID and try again.</p>
    <button onClick={() => { setBookingNotFound(false); setSearchValue(''); }}
      className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-red-600 text-white hover:bg-red-700 text-sm font-medium transition-all shadow-sm">
      Try Again
    </button>
  </div>
)}
{deletedBooking && (
  <div className="bg-red-50 border border-red-200 text-red-700 text-[15px] rounded-xl px-5 py-4 mb-6 text-center">
    ...
    <p className="font-semibold text-[17px] mb-1">Booking Removed</p>
    <p className="mb-4">This booking has been removed by the service center.</p>
    <div className="flex items-center justify-center gap-3">
      ...
      <a href={`https://wa.me/919948299426?text=${encodeURIComponent('Hi, I need help with a removed booking.')}`} target="_blank" rel="noopener noreferrer"
        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-green-600 text-white hover:bg-green-700 text-sm font-medium transition-all shadow-sm">
...
{!deletedBooking && !bookingNotFound && error && (
